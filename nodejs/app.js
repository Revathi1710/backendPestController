...
app.post('/VSearchBuyer', async (req, res) => {
  ...
  try {
    ...
    // Check if we should save the user
    if (!skipUserSave) {
      const existing = await User.findOne({ email });
      if (!existing) {
        const user = new User({ name, email, state, pincode, number, businessType });
        await user.save();
        userId = user._id;  // Capture the user ID when the user is created
      } else {
        userId = existing._id;  // Use the existing user's ID if found
      }
    } else {
      // If skipUserSave is true, no user saving is done, and we just use the existing user
      const existing = await User.findOne({ email });
      if (existing) {
        userId = existing._id;
      }
...
app.post('/addVendor', upload.fields([{ name: 'logo' }, { name: 'image', maxCount: 5 }]), async (req, res) => {
  try {
    const {
      businessName, address, pincode, sinceFrom, specialistIn,
    ...
    const cleanedData = {
      ...
      sinceFrom: sinceFrom.trim(),
      specialistIn: specialistIn.trim(),
      contactPerson: contactPerson.trim(),
      contactNumber: contactNumber.trim(),
      email: email.trim(),
      pesticideLicence: pesticideLicence.trim(),
      gstNumber: gstNumber.trim(),
      membership: membership.trim(),
      branchDetails: branchDetails.trim(),
      technicalQualification: technicalQualification.trim(),
    ...
    const encryptedPassword = await bcrypt.hash(cleanedData.password, 10);

    // Extract filenames
    const logoPath = logoFile.filename;
